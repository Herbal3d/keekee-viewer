...
namespace KeeKee {
public class KeeKeeBase : IInstance<KeeKeeBase> {
    // static things that are accessed very early (mostly by logger)
    private static string m_applicationName = "KeeKee";
    public static string ApplicationName { get { return m_applicationName; } }
    private static string m_applicationVersion = "V0.6.3";
    public static string ApplicationVersion { get { return m_applicationVersion; } }

    ILog m_log = LogManager.GetLogger("KeeKeeBase");

    private static KeeKeeBase m_instance = null;
    public static KeeKeeBase Instance {
        get {
            if (m_instance == null) {
                throw new KeeKeeException("FETCH OF LOOKINGGLASSBASE INSTANCE BEFORE SET!!!!");
            }
            return m_instance;
        }
    }
