...
public class BasicWorkQueue : IWorkQueue {
    ...
    public BasicWorkQueue(string nam) {
        ...
        if (KeeKeeBase.Instance.AppParams.HasParameter("Framework.WorkQueue.MaxThreads")) {
            m_workProcessorsMax = KeeKeeBase.Instance.AppParams.ParamInt("Framework.WorkQueue.MaxThreads");
        }
        if (KeeKeeBase.Instance.AppParams.HasParameter(m_queueName+".WorkQueue.MaxThreads")) {
            m_workProcessorsMax = KeeKeeBase.Instance.AppParams.ParamInt(m_queueName+".WorkQueue.MaxThreads");
        }
        lock (m_doEvenLater) {
    ...
    private void DoItEventLaterProcessing() {
        while (KeeKeeBase.Instance.KeepRunning) {
