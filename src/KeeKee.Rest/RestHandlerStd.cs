...
namespace KeeKee.Rest {
    ...
    public class RestHandlerStd : IRestHandler {
        ...
        public const string BaseUrl = "std/";

        // The prefix of the requested URL that is processed by this handler.
        public string Prefix { get; set; }

        public RestHandlerStd(KLogger<RestHandlerStatic> pLogger,
            ...
            if (pRequest?.HttpMethod.ToUpper().Equals("GET") ?? false) {
                ...
                try {
                    if (File.Exists(filePath)) {
                        _log.Log(KLogLevel.RestDetail, "RestHandlerStatic: Serving file {0}", filePath);
                        _RestManager.DoSimpleResponse(pResponse, Utilities.GetMimeTypeFromFileName(filePath), () => {
                            return File.ReadAllBytes(filePath);
                        }
                        );
                    } else {
                        _log.Log(KLogLevel.RestDetail, "RestHandlerStatic: File not found {0}", filePath);
                        _RestManager.DoErrorResponse(pResponse, HttpStatusCode.NotFound, null);
                    }
                } catch (Exception e) {
                    _log.Log(KLogLevel.Error, "RestHandlerStatic: Exception {0} serving file {1}", e.Message, filePath);
