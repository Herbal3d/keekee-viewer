...
namespace KeeKee.View {
    public partial class ViewWindow : IViewWindow {
        ...
        public ViewWindow(KLogger<ViewWindow> pLog,
            ...
            if (subControls.Length == 1) {
                ...
                m_log.Log(KLogLevel.DBADERROR, "Could not find window control on dialog");
        ...
        public void Initialize() {
            try {
                ...
                m_framesPerSec = Math.Min(100, Math.Max(1, m_ViewConfig.Value.FramesPerSecond));
                m_frameTimeMs = 1000 / m_framesPerSec;
                m_frameAllowanceMs = Math.Max(m_framesPerSec - 30, 10);
                m_log.Log(KLogLevel.DVIEWDETAIL, "Initialize. Connecting to renderer {0} at {1}fps",
        ...
        public void LGWindow_Paint(object sender, PaintEventArgs e) {
            try {
                ...
                m_log.Log(KLogLevel.DBADERROR, "LGWindow_Paint: EXCEPTION: {0}", err);
            }
