...
namespace KeeKee.World.OS {
public class OSAssetContextCap : AssetContextBase {
    ...
    public OSAssetContextCap(string name, Uri baseUri) : base(name) {
        m_baseUri = baseUri;
    }

    public override void InitializeContextFinish() {
        if (m_baseUri == null) {
            m_log.Log(LogLevel.DBADERROR, "OSAssetContextCap::InitializeContextFinish: NOT BASE PATH SPECIFIED: NOT INITIALIZING");
            return;
        }
